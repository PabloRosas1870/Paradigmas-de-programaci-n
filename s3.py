...
class S3 (RepositorioDeUsuarios):
	...
	def __init__ (mi, clientId:str,secretKey:str,bucket:str):
		...
		mi.__ secretKey = secretKey
		mi.__ bucket = bucket
		
	def abrir (mi) -> None :
		print (f "Estableciendo conexion a AWS S3 {mi.__clientId}:{mi.secretKey}")
	def guardar (mi, usuario:Usuario)-> None:
		userData ={"nombre": usuario.getNombre(),
		"apellido":usuario.getApellido(),
		"edad": usuario.getEdad()}
		print (f "Guardando usuario en la bandeja :{mi.__bucket}:{userData}")
