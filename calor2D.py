...
#numero de celsa
n=np.array([512,512])
#Tmaño del dominio (menor que uno)
L= np.array([1.0,1.0])
#constante de la difusión
k =0.2
#Pasos de tiempo 
pasos =100
#=======================================
...
un=np.zeros(nt,dtype=np.float64) #Arreglo de escr...
...
def evolucion (u,n,udx2,dt,i,k):
	...
	return unueva
def solución (u,un,udx2,dt,n,k):
	for jj in range (1,n[1]-1):
		for ii in range (1,n[0]-1):
			i = ii +n [0]*jj
			unueva= evolucion(u,n,udx2,dt,ik)
...
for t in range (1, pasos+1):
	...
	u = un 
	if t%100==0: print ("Iterracion=",t)
end =time.time()

print("Se tardó" , end-start."s")

u = np.reshape(u,(n[0], n[1]))
x,y = np.meshgrid(np.aragne(0,L[0],dx[0]),np.aragne(0,L[1],dx[1]))
ax=plt.axes(projection='3d')
sx.plot_surface(x,y,u,cmap=cm.hsv)
plt.show()
