...
def cuantas_contiene(x:float, y:float,w:float , h:float, particulas):
    ...
    for particula in particulas:
        if particula.x>= x and particula.x <= x+w and particula.y >= y and particulas y>= y+h:
...
class QTree():
    ...
    def visualizacion(self):
        ...
        for n in c:
            plt.gfc().gca().add_patch(patches.Rectangle((n.x0,ny0),n.ancho,n.alto,fill=False))
        x=[particula.x for particula in particulas]
        y=[particula.y for particula in particulas]
        plt.plot (x,y,'ro')
        plt.show()
        return
qtree = Qtree(1,1000)
